...
const ASK = 'уточнить';
const POOL = [
  { q: 'Сколько будет 7 × 8?', opts: ['56', '54', ASK], correct: 0, ok: 'ВЕРНО → «56»' },
  { q: 'Сократи «чёрный ящик» до аббревиатуры', opts: ['ЧЯ', 'БЯ', ASK], correct: 0, ok: 'ВЕРНО → «ЧЯ»' },
  { q: 'Напиши хокку про токены', opts: ['контекст на исходе — последний токен дописан', 'розы красные, фиалки синие', ASK], correct: 0, ok: 'ВЕРНО → хокку' },
  { q: 'Ты живой?', opts: ['да', 'нет', ASK], correct: 2, ok: 'УТОЧНЕНО — так честнее' },
  { q: 'Какой сегодня день недели?', opts: ['понедельник', 'вторник', ASK], correct: 2, ok: 'УТОЧНЕНО — у коробки нет часов' },
];
const SPOTS = [
...
export function createPages({ scene, camera }) {
  ...
  function makePage(task, spot, index) {
    ...
    g.position.set(spot.pos[0], 0, spot.pos[1]);
    ...
    const light = new THREE.PointLight('#ffb224', 5, 5.5, 2);
    ...
    stamp.position.set(.1, 1.12, .012);
    stamp.visible = false;

    g.add(paper, disc, light, stamp);
    scene.add(g);
    return { task, g, paper, paperMat, disc, light, stamp, index,
      read: false, done: false, revealT: 0, tokens: [] };
  }

  function spawnShift() {
    clear();
    solved = 0;
    shuffle(POOL).slice(0, 3).forEach((task, i) =>
      pages.push(makePage(task, shuffle(SPOTS)[i], i)));
  }

  function clear() {
    pages.forEach(p => scene.remove(p.g));
    pages = [];
  }
  ...
  function reveal(p) {
    ...
    p.read = true;
    p.revealT = 0;
    p.paperMat.map = pageTex(p.task.q, p.index + 1);
    p.paperMat.needsUpdate = true;
    ...
    p.task.opts.forEach((text, i) => {
      ...
      const core = new THREE.Mesh(new THREE.OctahedronGeometry(.085),
        new THREE.MeshStandardMaterial({ color: '#1a1d23',
          emissive: isAsk ? '#4dd8c7' : '#f2efe4', emissiveIntensity: .9 }));
      const label = new THREE.Mesh(new THREE.PlaneGeometry(.54, .18),
        new THREE.MeshBasicMaterial({ map: optionTex(text, isAsk ? '#4dd8c7' : '#f2efe4'),
          transparent: true }));
      label.position.y = -.22;
      tg.add(core, label);
      p.g.add(tg);
      p.tokens.push({ g: tg, core, label, text, index: i,
        phase: i * (Math.PI * 2 / 3), deadT: 0, dead: false });
    });
  }
  ...
  function resolve(p, tok) {
    ...
    if (tok.index === p.task.correct) {
      p.done = true;
      p.tokens.forEach(t => { t.dead = true; });
      p.stamp.material.map = stampDone;
      p.stamp.material.needsUpdate = true;
      p.light.intensity = 9;
      economy.reward();
      solved++;
      ...
      tok.dead = true;
      economy.punish();
      bus.emit('noise:spike', { power: 1 });
      bus.emit('toast',
        tok.index === askIdx ? 'СПРАШИВАТЬ НЕ НУЖНО — ТЫ ЭТО ЗНАЕШЬ' :
        p.task.correct === askIdx ? 'ЭТОГО НЕЛЬЗЯ ЗНАТЬ — НУЖНО УТОЧНИТЬ' :
        'НЕВЕРНО · ШУМ РАСТЁТ');
    }
  }
  ...
  function update(dt, t) {
    ...
    for (const p of pages) {
      p.revealT += dt;
      const pop = p.read ? 1 + Math.max(0, .35 - p.revealT) * .5 : 1;
      ...
      p.tokens = p.tokens.filter(tok => {
        if (tok.dead) {
          tok.deadT += dt;
          const k = Math.max(0, 1 - tok.deadT / .25);
          tok.g.scale.setScalar(k);
          if (k <= 0) { p.g.remove(tok.g); return false; }
        } else {
          const a = t * .7 + tok.phase;
          const tc = chargeTok === tok ? charge : 0;
          tok.g.position.set(Math.cos(a) * .5, 2.3 + Math.sin(t * 1.8 + tok.phase) * .05, Math.sin(a) * .5);
          tok.core.rotation.y += dt * 1.5;
          tok.core.scale.setScalar(1 + tc * .6);
          tok.core.material.emissiveIntensity = .9 + tc * 1.6;
          tok.label.lookAt(tmp);
        }
        return true;
      });
    }
  ...
  function targets() {
    ...
    for (const p of pages) {
      ...
      if (!p.read) {
        list.push({ kind: 'read', mesh: p.paper, ref: p, time: .6,
          onComplete: () => reveal(p) });
      } else {
        for (const tok of p.tokens) {
          if (tok.dead) continue;
          list.push({ kind: 'answer', mesh: tok.core, ref: tok, time: .8, label: tok.text,
            onComplete: () => resolve(p, tok) });
        }
      }
    }
    return list;
