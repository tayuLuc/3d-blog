...
export function createHud() {
  ...
  function paintTokens({ tokens, max, low }) {
  ...
  function showPhase(p) {
    ...
    phaseName.style.color = p.color || '';
    pips.innerHTML = '';
    for (let i = 0; i < p.total; i++) {
      const s = document.createElement('i');
      if (i < p.done) s.className = 'done';
      pips.appendChild(s);
    }
  }
  ...
  bus.on('tokens:empty', () => {
    ...
    dryT = n;
    showToast('КОНТЕКСТ ПУСТ — ЖДИ ПОПОЛНЕНИЯ');
    cross.classList.add('dry');
    setTimeout(() => cross.classList.remove('dry'), 300);
  });
  bus.on('lock:change', ({ locked }) => document.body.classList.toggle('locked', locked));

  paintTokens(economy.snapshot());
  scoreEl.textContent = economy.snapshot().solved;
}
