...
export function createRoom(scene) {
  ...
  roomMesh.position.y = LAYOUT.size[1] / 2; scene.add(roomMesh);
  ...
  edges.position.copy(roomMesh.position); scene.add(edges);
  ...
  .forEach(([w,h,d,px,py,pz]) => { const m = new THREE.Mesh(new THREE.BoxGeometry(w,h,d), stripMat); m.position.set(px,py,pz); scene.add(m); });
  ...
  const slotLight = new THREE.PointLight('#ffb224',26,13,2); slotLight.position.set(slot.x,2.6,-4.4); scene.add(slotLight);
  const trayLight = new THREE.PointLight('#4dd8c7',12,11,2); trayLight.position.set(tray.x,1.4,-4.4); scene.add(trayLight);
  const fill = new THREE.PointLight('#5a6f8a',9,18,2); fill.position.set(0,3.3,4.6); scene.add(fill);
  ...
  return {
    ...
    update(dt, t, active) {
      ...
      const e = easeOut(openT); shellMat.transparent = e > 0; shellMat.opacity = 1 - e * .88; shellMat.color.lerpColors(cFrom, cTo, e); scene.fog.far = 26 + e * 44;
