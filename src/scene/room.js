...
const LAYOUT = {
  size: [10, 4.5, 12], slot: { x: -2.2, y: 2.45 }, tray: { x: 2.2, y: 1.15 },
  capsule: { spawn: [-2.2, 2.45, -5.4], hover: [-.55, 1.8, -3.3], solve: [2.2, 1.15, -5.5] },
  trayHit: { x: 2.2, y: 1.15, halfX: .85, halfY: .62, z: -5.7 },
};
const CFG = { dustCount: 240, openDuration: 1.4, hlLerp: 5, trayPulse: { glow: 2.5, decay: 3, lightBoost: 40 } };
...
export function createRoom(scene) {
  ...
  roomMesh.position.y = LAYOUT.size[1] / 2; group.add(roomMesh);
  ...
  [[9.6,.05,.05, 0,3.55,-5.92],[9.6,.05,.05, 0,3.55,5.92],[.05,.05,11.7, -4.92,3.55,0],[.05,.05,11.7, 4.92,3.55,0]]
  .forEach(([w,h,d,px,py,pz]) => { const m = new THREE.Mesh(new THREE.BoxGeometry(w,h,d), stripMat); m.position.set(px,py,pz); group.add(m); });
  ...
  const slotFrame = new THREE.Mesh(new THREE.BoxGeometry(1.7, 1.05, .12), new THREE.MeshStandardMaterial({ color: '#0a0c0f', roughness: .9 }));
  slotFrame.position.set(slot.x, slot.y, -5.9); group.add(slotFrame);
  const slotSlit = new THREE.Mesh(new THREE.PlaneGeometry(1.25, .13), slotSlitMat);
  slotSlit.position.set(slot.x, slot.y, -5.83); group.add(slotSlit);
  const slotSign = new THREE.Mesh(new THREE.PlaneGeometry(1.7, .37), new THREE.MeshBasicMaterial({ map: signTex('ВХОД // REQUEST','#ffb224') }));
  slotSign.position.set(slot.x, 3.28, -5.92); group.add(slotSign);

  const trayGlowMat = new THREE.MeshStandardMaterial({ color: '#061a18', emissive: '#4dd8c7', emissiveIntensity: .85 });
  const trayFrame = new THREE.Mesh(new THREE.BoxGeometry(1.6, 1.1, .12), new THREE.MeshStandardMaterial({ color: '#0a0c0f', roughness: .9 }));
  trayFrame.position.set(tray.x, tray.y, -5.9); group.add(trayFrame);
  const trayGlow = new THREE.Mesh(new THREE.PlaneGeometry(1.35, .85), trayGlowMat);
  trayGlow.position.set(tray.x, tray.y, -5.83); group.add(trayGlow);
  const trayShelf = new THREE.Mesh(new THREE.BoxGeometry(1.7, .08, .4), new THREE.MeshStandardMaterial({ color: '#0a0c0f', roughness: .9 }));
  trayShelf.position.set(tray.x, .58, -5.72); group.add(trayShelf);
  const traySign = new THREE.Mesh(new THREE.PlaneGeometry(1.7, .37), new THREE.MeshBasicMaterial({ map: signTex('ВЫХОД // RESPONSE','#4dd8c7') }));
  traySign.position.set(tray.x, 2.0, -5.92); group.add(traySign);

  group.add(new THREE.HemisphereLight('#8fa0b5','#0b0d11',.5));
  const slotLight = new THREE.PointLight('#ffb224',26,13,2); slotLight.position.set(slot.x,2.6,-4.4); group.add(slotLight);
  const trayLight = new THREE.PointLight('#4dd8c7',12,11,2); trayLight.position.set(tray.x,1.4,-4.4); group.add(trayLight);
  const fill = new THREE.PointLight('#5a6f8a',9,18,2); fill.position.set(0,3.3,4.6); group.add(fill);
  const dp = new Float32Array(CFG.dustCount*3);
  for(let i=0;i<CFG.dustCount;i++){dp[i*3]=(Math.random()*2-1)*4.7;dp[i*3+1]=.2+Math.random()*4;dp[i*3+2]=(Math.random()*2-1)*5.6;}
  const dustGeo = new THREE.BufferGeometry(); dustGeo.setAttribute('position',new THREE.BufferAttribute(dp,3));
  group.add(new THREE.Points(dustGeo,new THREE.PointsMaterial({color:'#8b96a5',size:.02,transparent:true,opacity:.5,depthWrite:false})));
  ...
  return {
    ...
    update(dt, t, active) {
      ...
      trayGlowMat.emissiveIntensity += trayCharge * 1.8;
