...
export function createLoop({ parent, camera }) {
  ...
  function update(dt, t, activeHL) {
    ...
    const glow = Math.sin(t * 3.6) * .5 + .5; idle += dt;
    ...
    nodes.forEach((n, i) => { const a = angle(i);
      ...
      const want = (isActive ? 1.2 + nodeCharge * 1.7 : .5) + (isActive ? (Math.sin(t * 5) * .5 + .5) * .6 : 0) + hlSteps * glow * 1.4 + fizzle[i] * 2.5;
      ...
      n.core.scale.setScalar(n.core.scale.x + ((isActive ? 1.15 + nodeCharge * .35 : 1) - n.core.scale.x) * Math.min(1, dt * 8));
    });
  ...
  return { show, hide, setDemo, reset, update, activeNode, idleNodes, advance, doFizzle, startTask, isActive };
}
